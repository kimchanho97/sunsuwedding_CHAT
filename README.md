...
### 1️⃣ 트래픽 특성에 맞는 저장소 설계

채팅 서비스는 데이터 특성과 트래픽 패턴에 따라 최적화된 다중 저장소 아키텍처를 채택했습니다.

#### 저장소별 핵심 역할

- **MongoDB**: 채팅 메시지 전용 저장소
    - 1:1에 가까운 read/write 비율 처리에 최적화
    - 유연한 스키마로 다양한 메시지 타입 통합 관리
    - `채팅방ID-시퀀스ID` 복합 인덱스로 조회 성능 최적화
- **RDB**: 영구적 메타데이터 관리
    - 채팅방, 참가자 정보 등 비즈니스 엔티티 저장
    - 백엔드 서비스와의 데이터 통합성 보장
- **Redis**: 고성능 실시간 데이터 처리
    - 채팅방 메타데이터, 읽음 상태, 접속 상태 등 빈번히 변경되는 데이터 고속 처리
    - 채팅방 시퀀스 번호 생성 및 사용자별 채팅방 최신순 정렬 관리

<br>

### 2️⃣ Redis 고성능 실시간 처리

채팅 서비스의 실시간 데이터는 Redis에 우선 저장하여 지연 시간을 최소화하고 처리량을 극대화합니다.

#### 데이터 관리 원칙

- **Write-Back 방식**: 채팅 메타데이터는 Redis에 먼저 저장 후 이벤트 발행 및 스케줄러를 통해 영구 저장소에 비동기 동기화
- **Read-Aside 캐싱**: 자주 조회되는 데이터는 Redis에서 먼저 조회, 없으면 RDB에서 조회 후 Redis에 저장
- **독립 저장소 활용**: 접속 상태(Presence)와 같은 일부 임시 데이터는 Redis만을 저장소로 활용, TTL 설정으로 자동 관리
- **메모리 최적화**: 효율적인 자료구조와 키 설계를 통한 메모리 사용 최소화 (HASH, ZSET 적극 활용)
...
### 3️⃣ 데이터 일관성 관리 (Eventual Consistency)
...
- **변경 추적 방식**:
    - Redis 내 변경 키를 전용 SET에 기록 (`dirty:*` 키 사용)
    - `dirty:chat:room:meta`, `dirty:chat:room:last-read` 등으로 동기화 대상 관리
- **배치 동기화 처리**:
    - 주기적 스케줄러를 통한 청크 단위 배치 처리 (3분 간격, 최대 100건)
    - JDBC 배치 업데이트로 DB 부하 최소화

#### 동기화 파이프라인

1. **변경 감지**: Redis SET에 변경된 키 ID 저장
2. **정기 수집**: 스케줄러가 dirty 키 집합 스캔
3. **청크 처리**: 지정된 크기(100개)로 키 추출
4. **일괄 갱신**: 백엔드 API를 통한 영구 저장소 업데이트
5. **상태 정리**: 성공 처리된 키는 추적 목록에서 제거
...
- **Redis INCR 기반 시퀀스**:
    - 채팅방별 단조 증가 시퀀스로 메시지 순서 보장
    - 시퀀스 ID 기반 읽음 상태 및 미읽음 카운트 계산

#### 이벤트 기반 동기화

- **Kafka 이벤트 스트리밍**:
    - 메시지 저장 후 연쇄적 이벤트 발행으로 시스템 간 상태 전파
    - 멱등적 처리(동일 이벤트 중복 처리 방지)를 통해 일관성 확보

#### 데이터 서비스 계층화

- **`RedisService` 추상화**: 저장소 접근 로직 캡슐화
    - 자동 폴백 매커니즘: Redis 실패 시 RDB 조회
    - 일관된 데이터 액세스 패턴 제공
    - 저장소 구현 변경에 대한 영향 최소화
